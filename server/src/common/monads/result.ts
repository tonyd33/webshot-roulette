...
import { curry1, isVoid, safeToString } from './utils';
...
class ResultImpl<T, E> {
  ...
  map<U>(mapFn: (t: T) => U): Result<U, E> {
    return (
      this.repr[0] === 'Ok' ? Result.ok(mapFn(this.repr[1])) : this
    ) as Result<U, E>;
  }
  ...
  match<U>(matcher: Matcher<T, E, U>): U {
    return this.repr[0] === 'Ok'
      ? matcher.Ok(this.repr[1])
      : matcher.Err(this.repr[1]);
  }

  /** Method variant for {@linkcode mapErr} */
  mapErr<F>(mapErrFn: (e: E) => F): Result<T, F> {
    return (
      this.repr[0] === 'Ok' ? this : Result.err(mapErrFn(this.repr[1]))
    ) as Result<T, F>;
  }
  ...
  orElse<F>(orElseFn: (err: E) => Result<T, F>): Result<T, F> {
    return (this.repr[0] === 'Ok' ? this : orElseFn(this.repr[1])) as Result<
      T,
      F
    >;
  }
  ...
  andThen<U>(andThenFn: (t: T) => Result<U, E>): Result<U, E> {
    return (this.repr[0] === 'Ok' ? andThenFn(this.repr[1]) : this) as Result<
      U,
      E
    >;
  }
  ...
  toJSON(): ResultJSON<T, E> {
    ...
    return variant === 'Ok'
      ? { variant, value: this.repr[1] }
      : { variant, error: this.repr[1] };
  }
