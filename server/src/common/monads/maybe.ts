...
class MaybeImpl<T> {
  ...
  */
  ...
  static of<T extends {}, F extends (...args: any) => T | null | undefined>(
    value: F,
  ): never;
  ...
  static just<T extends {}, F extends (...args: any) => T | null | undefined>(
    value: F,
  ): never;
  ...
  map<U extends {}>(mapFn: (t: T) => U): Maybe<U> {
    return (
      this.repr[0] === 'Just' ? Maybe.just(mapFn(this.repr[1])) : this
    ) as Maybe<U>;
  }
  ...
  andThen<U>(andThenFn: (t: T) => Maybe<U>): Maybe<U> {
    return (
      this.repr[0] === 'Just' ? andThenFn(this.repr[1]) : this
    ) as Maybe<U>;
  }
  ...
  toJSON(): MaybeJSON<unknown> {
    ...
    if (variant === 'Just') {
      ...
      const value = isInstance(this.repr[1])
        ? this.repr[1].toJSON()
        : this.repr[1];
